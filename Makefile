...
.PHONY: help install precommit dataset notebook lint format test check clean
...
# ---------- Запуск ----------
dataset: ## Сгенерировать синтетический датасет data/events.csv
	$(PYTHON) src/lab04/make_dataset.py

notebook: ## Открыть основной ноутбук в Jupyter
	jupyter lab notebooks/lab04.ipynb
